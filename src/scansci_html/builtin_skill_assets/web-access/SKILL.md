...
Upstream source: https://github.com/eze-is/web-access
...
| 场景 | 工具 |
|------|------|
| 搜索摘要或关键词结果，发现信息来源 | **`search_web`** |
| 已知公开 URL、RSS、GitHub、Bilibili、V2EX，需要只读获取内容 | **`agent_reach`** |
| 已知 URL，但公开读取遇到登录墙、反爬或动态内容不足 | **`browser_access`**（只读渲染页面） |
| 需要点击、填写、提交、上传或其他页面交互 | **当前 Pi 只读边界不覆盖；不要把 `browser_access` 当成交互工具** |

`browser_access` 由 ScanSci 通过内置 CDP Proxy 执行受限读取，会创建后台 tab 并在完成后关闭；它不暴露任意 `/eval`、点击或写入能力。需要完整页面交互时，应明确说明当前 Pi 只提供只读浏览器桥接，不要虚构已完成交互。
