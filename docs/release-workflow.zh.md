...
> 发布渠道分级：`beta` 仅生成受邀内测包；`release` 才是需要公开信任代码签名和人工桌面验收的正式候选包。两者不得混用。
...
- 由受信任代码签名证书签署 `ScanSci.exe` 和安装包，并复核 Authenticode 为 `Valid`；
- 完成同一候选的安全扫描与高风险问题处置；
- 由网关维护者确认生产凭据、限流、监控与故障响应可用；
- 发布不可变下载地址、SHA256、版本说明和回滚方案。

没有这些外部证据时，只能称为“已验证的发布候选”，不得称为公开正式版。
...
没有公开信任代码签名证书时，只能发布给受邀测试者，不能把包放到公开下载页。使用独立的 `beta` 档，而不是降低 `release` 档的签名要求：
...
正式门禁在任何耗时构建前先检查签名环境；未配置时会失败，不会产出未签名的“正式版”安装包。发布机必须具备：
