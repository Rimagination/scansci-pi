...
if ($ValidateOnly) {
    Write-Output "ScanSci gateway setup is ready: $GatewayDirectory"
    exit 0
}
...
$form.Text = "ScanSci gateway secret"
...
$cancel.Text = "Cancel"
$cancel.Location = New-Object System.Drawing.Point(380, 94)
...
try {
    $apiKey | & npx wrangler secret put ZHIPU_API_KEY
    if ($LASTEXITCODE -ne 0) {
        throw "Cloudflare rejected the gateway secret."
    }
}
finally {
    Pop-Location
...
[System.Windows.Forms.MessageBox]::Show(
    "The upstream key is now stored as a Cloudflare Worker Secret. It is not included in ScanSci or this repository.",
    "ScanSci gateway",
